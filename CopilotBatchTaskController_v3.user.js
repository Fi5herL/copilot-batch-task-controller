// ==UserScript==
// @name         Copilot Batch Task Controller v3
// @namespace    http://tampermonkey.net/
// @version      3.0
// @description  批次 Copilot 任務控制器：多分頁管理、縮小狀態列、網頁截取送入 (M365 修正版)
// @author       Fi5herL
...
(function () {
    ...
    if (isCopilotPage && isSlavePage) { initSlaveMode(); return; }
    if (isCopilotPage) { initMasterUI(); return; }
    initCaptureButton();
    ...
    function initCaptureButton() {
        ...
        btn.id = 'cbtc-capture-btn';
        btn.title = '截取網頁內容送入 Copilot';
        btn.textContent = '📎';
        btn.style.cssText = `
            position:fixed;bottom:16px;right:20px;z-index:2147483647;
        ...
        menu.style.cssText = `
            ...
            box-shadow:0 6px 24px rgba(0,0,0,.5);border:1px solid rgba(255,255,255,.2);min-width:180px;
        `;
        menu.innerHTML = `
            <div id="cbtc-cap-selected" style="padding:8px;border-radius:7px;cursor:pointer;margin-bottom:4px;background:rgba(255,255,255,.1)">✂️ 截取選取文字</div>
            <div id="cbtc-cap-page" style="padding:8px;border-radius:7px;cursor:pointer;background:rgba(255,255,255,.1)">📄 截取整頁文字</div>
        `;
